...
if (!nv_function_exists('nv_menu_top_header')) {
    /**
     * nv_menu_top_header_check_current()
     *
     * @param mixed $url
     * @param integer $type
     * @return
     *
     */
    function nv_menu_top_header_check_current($url, $type = 0)
    {
     ...
     *
     */
    function nv_menu_top_header($block_config)
    {
        global $nv_Cache, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid, $home;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/menu/global.top_header.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/menu/global.top_header.tpl')) {
        ...
        $xtpl = new XTemplate('global.top_header.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu');
        ...
        if (!empty($array_menu)) {
            foreach ($array_menu[0] as $id => $item) {
                ...
                if (nv_menu_top_header_check_current($item['link'], $item['active_type'])) {
                    $classcurrent[] = 'active';
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_top_header($block_config);
}
